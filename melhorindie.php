...
$games = [
    [
        'id' => 7,
        'title' => 'BALATRO',
        'developer' => 'Local Thunk',
        'status' => 'closed',
        'image' => 'imagens/balatro.jpg'
    ],
    [
        'id' => 2,
        'title' => 'ANIMAL WELL',
    ...
    [
        'id' => 3,
        'title' => 'FIVE NIGHTS AT FREDDYS: INTO THE PIT',
        'developer' => 'Scottgames',
        'status' => 'closed',
        'image' => 'imagens/fnaf.avif'
    ],
    [
        'id' => 5,
        'title' => 'NEVA',
        'developer' => 'Devolver',
        'status' => 'closed',
        'image' => 'imagens/neva.jpg'
    ],
    [
        'id' => 5,
        'title' => 'O ESCUDEIRO VALENTE',
        'developer' => 'Devolver',
        'status' => 'closed',
        'image' => 'imagens/escudeiro.webp'
    ],
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['voto_id'], $_POST['nome'])) {
    ...
    if (!empty($nomeUsuario) && !empty($gameTitle)) {
        ...
        $stmt = $pdo->prepare("INSERT INTO votos_melhorindie (game_title, user_name, voted_at) VALUES (:game_title, :user_name, NOW())");
        $stmt->execute([
...
<body>
    <h1>Vote no Melhor Jogo Indie</h1>

    <!-- Botão próximo no topo -->
    <button class="next-button" onclick="window.location.href = 'melhorluta.php';">Próximo</button>
