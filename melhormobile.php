...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['voto_id'], $_POST['nome'])) {
    ...
    if (!empty($nomeUsuario) && !empty($gameTitle)) {
        ...
        $stmt = $pdo->prepare("INSERT INTO votos_melhormobile (game_title, user_name, voted_at) VALUES (:game_title, :user_name, NOW())");
        $stmt->execute([
...
<body>
    <h1>Vote no Melhor Jogo Mobile</h1>

    <!-- Botão próximo no topo -->
    <button class="next-button" onclick="window.location.href = 'melhormulti.php';">Próximo</button>
